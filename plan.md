...
| 4 | Пасив тривалий (`are testing` → `is being tested`) — ⚠️ **лише розпізнавання**, поки немає власного уроку (чекає на L24) | «предмет сам це робить чи з ним роблять?» |
...
> ⚠️ «Перевіряються в кожному раунді» = помічаємо, коли леак сам випливає у вільному продукуванні студента, АБО коли граматика вже пройдена окремим уроком. **Не форсуємо переклад/продукування конструкції, якої ще не було у `grammar/`** (напр. леак №4 — до L24 лише помічаємо пасив тривалий, якщо він сам зʼявився у власному тексті студента; спеціально вставляти `is being + V3` в завдання-переклад ІНШОГО уроку — не треба).

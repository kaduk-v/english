...
## accidentally
**accidentally** /ˌæksɪˈdentəli/ — <code>adverb</code> (прислівник) — випадково, ненавмисно
...
## accrue
**accrue** /əˈkruː/ — <code>verb</code> (дієслово) — накопичуватися, наростати (про відсотки, борг, вигоди)
...
## accuracy
**accuracy** /ˈækjərəsi/ — <code>noun</code> (іменник) — точність, влучність
...
## across
**across** /əˈkrɒs/ — 1. <code>preposition</code> (прийменник) 2. <code>adverb</code> (прислівник) — через, на той бік; по всій площині

**Пояснення:** рух або розташування з одного боку на інший **по поверхні** (дорога, міст, річка) або «по всьому» (across the country — по всій країні).

**💡 across vs through:**
- **across** — по поверхні, з боку на бік: swim **across** the river (пливти через річку — по поверхні).
- **through** — крізь, усередині простору: walk **through** the forest (іти через ліс — поміж дерев).
...
## actually
**actually** /ˈæktʃuəli/ — <code>adverb</code> (прислівник) — насправді, взагалі-то, власне

**Пояснення:** підкреслює реальний факт або м'яко виправляє співрозмовника. Часто стоїть на початку речення, коли хочеш уточнити чи заперечити.

**💡 False friend:**
- **actually** ≠ «актуально»! Це «насправді / взагалі-то».
...
## addict
**addict** /ˈædɪkt/ — <code>noun</code> (іменник) — залежна людина

**Пояснення:** людина, яка не може відмовитися від чогось — від наркотиків до ігор чи кави (часто жартівливо: a coffee addict). Наголос у іменника — на **перший** склад.

**💡 Сім'я слова:**
- **an addict** — особа: He's a game **addict**. — Він ігроман.
- **addicted to** + іменник/герундій — залежний від: I'm **addicted to** this game.
...
## addition
**addition** /əˈdɪʃn/ — <code>noun</code> (іменник) — додавання; доповнення, поповнення

**Пояснення:** математична дія (додавання) або нова річ/людина, що додалася (an addition to the team). Найчастіше трапляється у звороті **in addition (to)**.

**💡 in addition (to):**
- **in addition** — «до того ж, крім того» (формальніше за also).
...
## adjust
**adjust** /əˈdʒʌst/ — <code>verb</code> (дієслово) — регулювати, підлаштовувати; пристосовуватися

**Пояснення:** трохи змінити щось, щоб краще працювало або пасувало (гучність, яскравість, графік). Про людину — звикнути до нових умов.
...
## admit
**admit** /ədˈmɪt/ — <code>verb</code> (дієслово) — визнавати, зізнаватися; впускати

**Пояснення:** визнати неприємний факт або свою провину; формальне друге значення — впустити/прийняти (до закладу, лікарні). Після нього — герундій або `that`.

**💡 Патерни:**
- **admit doing** sth / **admit that**… — визнати, що: He **admitted breaking** the build.
...
## advantage
**advantage** /ədˈvɑːntɪdʒ/ — <code>noun</code> (іменник) — перевага

**Пояснення:** те, що ставить тебе у кращу позицію за інших. Протилежне — **disadvantage** (недолік).

**💡 Патерни:**
- **advantage of** sth / **over** sb — перевага чогось / над кимось.
...
## affect
**affect** /əˈfekt/ — <code>verb</code> (дієслово) — впливати на

**Пояснення:** діяти на щось і змінювати його. Вживається **без** прийменника: affect something (НЕ ~~affect on~~).

**💡 affect (v) vs effect (n) — класика:**
- **affect** — ДІЄСЛОВО «впливати»: The bug **affects** all users.
- **effect** — ІМЕННИК «ефект, наслідок»: The bug has an **effect on** all users.
- Мнемоніка: **A**ffect = **A**ction (дія), **E**ffect = **E**nd result (результат).
...
## affirm
**affirm** /əˈfɜːm/ — <code>verb</code> (дієслово) — стверджувати, підтверджувати (формально)
...
## afterwards
**afterwards** /ˈɑːftəwədz/ — <code>adverb</code> (прислівник) — потім, згодом, після того
...
## against
**against** /əˈɡenst/ — <code>preposition</code> (прийменник) — проти; об, до (опора); всупереч

**Пояснення:** протистояння (грати/боротися проти), фізична опора (притулити до стіни) або порушення норми (against the rules). У розробці — перевірка **за** еталоном: validate against a schema.

**💡 Сталі патерни:**
- play/fight **against** — грати/битися проти
- protect/defend **against** — захищати від
- lean **against** — спертися об
...
## aid
**aid** /eɪd/ — 1. <code>noun</code> (іменник) 2. <code>verb</code> (дієслово) — 1. допомога, підтримка 2. допомагати, сприяти (формально)
...
## ally
**ally** /ˈælaɪ/ — 1. <code>noun</code> (іменник) — союзник 2. <code>verb</code> (дієслово) — об'єднуватися в союз
...
## ammo
**ammo** /ˈæməʊ/ — <code>noun</code> (іменник, незлічуваний) — патрони, боєприпаси _(розмовне)_
...
## among
**among** /əˈmʌŋ/ — <code>preposition</code> (прийменник) — серед, поміж

**Пояснення:** усередині групи з трьох і більше або в оточенні чогось. Варіант **amongst** — те саме, трохи книжніше (BrE).

**💡 among vs between:**
- **between** — між ДВОМА (або чітко окремими) об'єктами: **between** the client and the server.
- **among** — серед ГРУПИ як цілого: popular **among** developers.
...
## ancient
**ancient** /ˈeɪnʃənt/ — <code>adjective</code> (прикметник) — стародавній, древній
...
## apart
**apart** /əˈpɑːt/ — <code>adverb</code> (прислівник) — окремо, нарізно; на відстані

**Пояснення:** про відстань між об'єктами у просторі чи часі або про розділення на частини: fall apart — розвалитися, take apart — розібрати.

**💡 apart from** — окрім, за винятком (= except for / besides):
- **Apart from** the login bug, the release is stable. — Окрім бага з логіном, реліз стабільний.
...
## apply
**apply** /əˈplaɪ/ — <code>verb</code> (дієслово) — 1. подавати заявку 2. застосовувати 3. наносити (крем тощо)
...
## appreciate
**appreciate** /əˈpriːʃieɪt/ — <code>verb</code> (дієслово) — цінувати, бути вдячним; усвідомлювати

**Пояснення:** бути вдячним за щось або розуміти цінність/складність чогось. Після нього йде іменник або герундій (не інфінітив).

**💡 Ввічливі патерни:**
- **I('d) appreciate it if** you could… — Я був би вдячний, якби ви… (класика робочих листів; не забудь **it**)
...
## appropriately
**appropriately** /əˈprəʊpriətli/ — <code>adverb</code> (прислівник) — доречно, відповідно, належним чином
...
## approximate
**approximate** — 1. <code>adjective</code> (прикметник) /əˈprɒksɪmət/ — приблизний, орієнтовний 2. <code>verb</code> (дієслово) /əˈprɒksɪmeɪt/ — приблизно оцінювати, наближатися (до значення)

**Пояснення:** прикметник — неточний, орієнтовний (approximate time/cost). Дієслово — оцінити приблизно або бути наближенням (математика, програмування).

**💡 Одне написання — дві вимови:**
- прикметник: /əˈprɒksɪ**mət**/ — закінчення звучить /-mət/: the **approximate** value.
- дієслово: /əˈprɒksɪ**meɪt**/ — закінчення звучить /-meɪt/: to **approximate** a function.
...
## arbitrary
**arbitrary** /ˈɑːbɪtrəri/ — <code>adjective</code> (прикметник) — довільний; свавільний
...
## arise
**arise** /əˈraɪz/ — <code>verb</code> (дієслово, неправильне) — виникати, поставати

**Пояснення:** з'являтися, ставатися — про абстрактне: проблеми, питання, труднощі, можливості. Формальніше за **come up**. Не плутай з **rise** (підніматися).

**💡 arise — arose — arisen:**
- Problems **arise**. — Проблеми виникають. (класична пара «problems/issues arise»)
- A conflict **arose** between the two guilds. — Між двома гільдіями виник конфлікт.
- New issues have **arisen** since the last deploy. — Після останнього деплою виникли нові проблеми.
...
## arrange
**arrange** /əˈreɪndʒ/ — <code>verb</code> (дієслово) — влаштовувати, домовлятися; розставляти, упорядковувати

**Пояснення:** організувати подію чи домовленість (arrange a meeting) або розкласти речі в певному порядку. Іменник — **arrangement** (домовленість; розташування).

**💡 Патерни:**
- **arrange** a meeting/call — призначити зустріч/дзвінок
- **arrange for** sb **to do** sth — домовитися, щоб хтось щось зробив
...
## assign
**assign** /əˈsaɪn/ — <code>verb</code> (дієслово) — призначати, доручати, розподіляти
...
## assignment
**assignment** /əˈsaɪnmənt/ — <code>noun</code> (іменник) — завдання; призначення; присвоєння
...
## attic
**attic** /ˈætɪk/ — <code>noun</code> (іменник) — горище
...
## attitude
**attitude** /ˈætɪtjuːd/ — <code>noun</code> (іменник) — ставлення, підхід; позиція
...
## aware
**aware** /əˈweə(r)/ — <code>adjective</code> (прикметник) — обізнаний, свідомий (чогось)

**Пояснення:** знати про існування чогось або помічати щось довкола. Стоїть після **be/become**, а не перед іменником. Протилежне — **unaware**.

**💡 aware of:**
- **be aware of** + щось — знати про: I'm **aware of** the risks.
...
## awful
**awful** /ˈɔːfl/ — <code>adjective</code> (прикметник) — жахливий, огидний

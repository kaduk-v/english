...
## rapid
**rapid** /ˈræpɪd/ — <code>adjective</code> (прикметник) — швидкий, стрімкий
...
## rare
**rare** /reə(r)/ — <code>adjective</code> (прикметник) — рідкісний; (про стейк) слабко просмажений, з кров'ю

**Пояснення:** такий, що трапляється дуже нечасто. Прислівник — **rarely** (рідко).

**💡 Два «побутові» значення:**
- **rare item** — рідкісний предмет: у більшості ігор градація common → **rare** → epic → legendary;
...
## rarely
**rarely** /ˈreəli/ — <code>adverb</code> (прислівник) — рідко

**Пояснення:** майже ніколи. Речення з rarely вже звучить як «майже не» — додаткове заперечення не потрібне (НЕ «I don't rarely play»).

**💡 Позиція в реченні:** перед смисловим дієсловом, але після `be` та допоміжних:
- I **rarely** play on weekdays.
- She **is rarely** late.
...
## rather
**rather** /ˈrɑːðə(r)/ — <code>adverb</code> (прислівник) — досить, доволі; краще, радше

**Пояснення:** три головні вжитки — вибір («я б краще»), протиставлення («а не») і ступінь («досить»).

**💡 Три конструкції:**
- **would rather** + інфінітив без to — я б краще: I'**d rather** stay home (than go out).
- **rather than** — а не, замість: We chose SQLite **rather than** Postgres.
...
## ray
**ray** /reɪ/ — <code>noun</code> (іменник) — промінь
...
## realize
**realize** /ˈrɪəlaɪz/ — <code>verb</code> (дієслово) — усвідомити, зрозуміти (часто раптом)

**Пояснення:** раптом зрозуміти факт або почати його розуміти. Часто **realize (that)** + підрядне речення. BrE також пишеться `realise`.
...
## refer
**refer** /rɪˈfɜː(r)/ — <code>verb</code> (дієслово) — звертатися (до джерела); стосуватися, означати

**Пояснення:** трохи формальне дієслово. У минулому часі подвоює `r`: referred, referring. Іменник — **reference** (посилання, довідка).

**💡 refer to** — завжди з `to`, два значення:
- звернутися по інформацію: **refer to** the docs / the manual;
...
**Пояснення:** відчувати смуток через те, що зробив (або не зробив). У минулому часі подвоює `t`: regretted.

**💡 Дві конструкції з різним значенням:**
- **regret + V-ing** — шкодую, що ЗРОБИВ (про минуле): I **regret buying** it.
...
## relatable
**relatable** /rɪˈleɪtəbl/ — <code>adjective</code> (прикметник) — близький, життєвий, «прямо про мене»
...
## rely
**rely** /rɪˈlaɪ/ — <code>verb</code> (дієслово) — покладатися, розраховувати (на когось/щось)

**Пояснення:** довіряти комусь/чомусь або залежати від нього. Прикметник — **reliable** (надійний).

**💡 rely on** — завжди з `on`:
- rely **on** sb/sth — покладатися на;
...
## remain
**remain** /rɪˈmeɪn/ — <code>verb</code> (дієслово) — залишатися
...
## rescue
**rescue** /ˈreskjuː/ — 1. <code>verb</code> (дієслово) — рятувати 2. <code>noun</code> (іменник) — порятунок
...
## research
**research** /rɪˈsɜːtʃ/ _(noun also /ˈriːsɜːtʃ/)_ — 1. <code>noun</code> (іменник, uncountable!) 2. <code>verb</code> (дієслово) — 1. дослідження 2. досліджувати
...
## respond
**respond** /rɪˈspɒnd/ — <code>verb</code> (дієслово) — відповідати, реагувати
...
**Пояснення:** продовжити щось після перерви: resume the game, resume work. Трохи формальне.

**💡 resume vs résumé — різна вимова й наголос!**
- **resume** /rɪˈzjuːm/ — дієслово «відновити»: press Start to **resume** the game;
...
## reveal
**reveal** /rɪˈviːl/ — <code>verb</code> (дієслово) — розкривати, показувати (те, що було приховане)
...
## rid
**rid** /rɪd/ — <code>verb</code> (дієслово) — позбавляти (вживається майже лише у фразі get rid of)

**Пояснення:** саме по собі слово майже не трапляється — вчи одразу цілу фразу.
...
## roll
**roll** /rəʊl/ — 1. <code>verb</code> (дієслово) — котити(ся); кидати (кубик) 2. <code>noun</code> (іменник) — кидок (кубика); рол, булочка; рулон

**Пояснення:** базове значення — котитися чи обертатися. Звідси і кидок кубика (він котиться), і рулон (скручений), і суші-рол.
...
## rope
**rope** /rəʊp/ — <code>noun</code> (іменник) — мотузка, канат

**Пояснення:** товстий міцний «шнур» із переплетених волокон (тонший — це string або cord).
...
## royal
**royal** /ˈrɔɪəl/ — <code>adjective</code> (прикметник) — королівський

**Пояснення:** пов'язаний із королем чи королевою: the royal family, the royal guard. У Британії the Royals — розмовно про королівську родину.
...
## rush
**rush** /rʌʃ/ — 1. <code>verb</code> (дієслово) — поспішати, мчати; підганяти 2. <code>noun</code> (іменник) — поспіх; наплив, пік

**Пояснення:** робити щось дуже швидко, часто через брак часу; або змушувати когось поспішати (rush sb).

**💡 Сталі фрази:**
- **in a rush** = in a hurry — поспіхом, у поспіху;
